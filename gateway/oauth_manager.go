...
func (o *OAuthManager) HandleAccess(r *http.Request) *osin.Response {
	...
	if ar := o.OsinServer.HandleAccessRequest(resp, r); ar != nil {
		...
		if username != "" && foundNewToken {
			...
			err := o.Gw.GlobalSessionManager.UpdateSession(keyName, session, session.Lifetime(o.API.RespectKeySessionLifetime, o.API.SessionLifetime, o.Gw.GetConfig().ForceGlobalSessionLifetime, o.Gw.GetConfig().GlobalSessionLifetime), false)
			if err != nil {
				log.Error(err)
			}
		}
